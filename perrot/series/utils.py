...
def crop_unsorted(data, crops, extend=False):
    """
...
def crop_sorted(data, crop, master, extend=False):
    """
    ...
    Args:
        ...
        master: int
            Index of the sorted data array to be used for indices determination.
...
def calc_limits_unsorted(data, crops, extend=False):
    """
    Crops given data by applying specified ranges and optionally extends
    ...
    data = crop_unsorted(data, crops, extend=extend)
    return calc_limits(*data)


def calc_limits_sorted(data, crop, extend=False, interpolate=False):
    """
    ...
    if extend or not interpolate:
        data = crop_sorted(data, crop, 0, extend=extend)
        return calc_limits(*data)
    ...
    # get cropped limits
    data = crop_sorted(data, crop, 0, extend=False)
...
def combine_limits(*limits):
    ...
    for i in range(size):
        values = [val for lim in limits if lim[i] is not None for val in lim[i]]
        final += [[min(values), max(values)]] if values else [None]
    
    if all(lim is None for lim in final):
        final = None
...
def calc_histogram(x, bins, minimum=None, maximum=None, base=None):
    """
    ...
    # calc cumulative sum
    cumsum = numpy.cumsum(hist)
    
    return bins, hist, cumsum
